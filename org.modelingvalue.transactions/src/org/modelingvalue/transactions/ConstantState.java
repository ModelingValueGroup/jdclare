    ...
    private class Constants<O> {
        ...
        private <V> V derive(AbstractLeaf leaf, O object, Constant<O, V> constant) {
            ...
            while (true) {
                try {
                    if (!list.isEmpty()) {
                        List<Pair<Object, Constant>> finalList = list;
                        WEAK.run(true, () -> {
                            for (Pair<Object, Constant> lazy : finalList) {
                                if (constant.equals(lazy.b()) && object.equals(lazy.a())) {
                                    Pair<Object, Constant> me = Pair.of(object, constant);
                                    throw new NonDeterministicException("Circular constant definition: " + finalList.sublist(finalList.lastIndexOf(me), finalList.size()).add(me));
                                }
